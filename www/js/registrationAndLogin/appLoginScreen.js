...
Game.appLoginScreen.prototype = {
	...
	startGame: function () {
		...
		var titleTxt = _this.add.text(_this.world.centerX - 40, 45, "Building Blocks G8 by Akshara");
		...
		_this.fish.events.onInputDown.add(function () {
			for (var i = 0; i < _this.userArray.length; i++) {
				if (_this.userArray[i].name.toLowerCase() == "fish") {
					...
					_this.state.start('appLoginEditScreen', true, false, _this.userArray[i], _this.app_Mode);
					return;
				}
			}
		}, this);
		...
		_this.tree.events.onInputDown.add(function () {
			for (var i = 0; i < _this.userArray.length; i++) {
				if (_this.userArray[i].name.toLowerCase() == "tree") {
					...
					//_this.state.start('index2',true,false,_this.user,false);//**If ONLINE 
					_this.state.start('appLoginEditScreen', true, false, _this.userArray[i], _this.app_Mode);//**Offline */
					return;
				}
			}
		}, this);
